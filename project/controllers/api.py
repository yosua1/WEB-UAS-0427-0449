...
def write_required(f):
    ...
    def wrap(*args, **kwargs):
        if 'x-write-key' in request.headers:
            user = UserModel.query.filter_by(write_key=request.headers['x-write-key']).first()
            if user:
...
def read_required(f):
    @wraps(f)
    def wrap(*args, **kwargs):
        if 'x-read-key' in request.headers:
            user = UserModel.query.filter_by(read_key=request.headers['x-read-key']).first()
            if user:
...
def api_news_by_id(id) -> dict:
    ...
    if not cekNews:
        return {"status" : 404, "msg" : "Berita Tidak Ditemukan"}
...
def api_create_news(user) -> dict:
    ...
    return {"status" : 200, "msg" : "Berhasil Memasukkan Berita"}
...
def api_edit_news(user, id) -> dict:
    cekNews = NewsModel.query.filter_by(id=id).first()
    if not cekNews:
        return {"status" : 404, "msg" : "Berita Tidak Ada"}
    ...
    db.session.commit()
    return {"status" : 200, "msg" : "Berhasil Mengedit Berita"}
...
def api_change_flag_news(user, id):
    ...
    db.session.commit()
    return {"status" : 200, "msg" : "Berhasil Mengedit Flag Berita"}
...
def api_delete_news(user, id):
    ...
    cekNews.delete()
    db.session.commit()
    return {"status" : 200, "msg" : "Berhasil Menghapus Data"}
