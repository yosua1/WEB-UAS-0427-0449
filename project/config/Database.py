...
create_db = False
if not os.path.isfile('db_akademik'):
    db.create_all()
    create_db = True


if create_db:
    user = UserModel(name='Afra-Hendra', username='admin', password=Hash().getHash('admin'), read_key=Hash().md5(os.urandom(32)), write_key=Hash().md5(os.urandom(32)), is_active=True)
    db.session.add(user)
    db.session.commit()